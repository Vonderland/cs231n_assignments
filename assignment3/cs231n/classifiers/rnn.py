...
class CaptioningRNN(object):
    ...
    def loss(self, features, captions):
        ...
        ############################################################################
        # the initial hidden state from the image features
...
#         dfeatures, grads['W_proj'], grads['b_proj'] = affine_backward(dh0, h0_cache)
        ...
        for i in range(max_length - 1):
            word_embed, word_cache = word_embedding_forward(captions[:,i], W_embed)
            if self.cell_type == 'rnn':
                prev_h, rnn_step_cache = rnn_step_forward(word_embed, prev_h, Wx, Wh, b)
            elif self.cell_type == 'lstm':
                prev_h, prev_c, lstm_step_cache = lstm_step_forward(word_embed, prev_h, prev_c, Wx, Wh, b)
            score, score_cache = affine_forward(prev_h, W_vocab, b_vocab)
            captions[:, i + 1] = np.argmax(score, axis=1)
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
        return captions
